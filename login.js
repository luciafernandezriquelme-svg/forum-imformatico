...
function registrarse() {
  ...
  if (!correo || !usuario || !clave) return mostrarMensaje("Completa todos los campos.");
  if (!esCorreoValido(correo)) return mostrarMensaje("Correo inválido.");

  const usuarios = JSON.parse(localStorage.getItem("usuariosRegistrados")) || [];
  if (usuarios.find(u => u.correo === correo)) return mostrarMensaje("Correo ya registrado.");
...
function iniciarSesion() {
  ...
  if (existe) {
    ...
    usuarios.push({ correo, usuario, clave: hash(clave) });
    localStorage.setItem("usuariosRegistrados", JSON.stringify(usuarios));
    mostrarMensaje("Registro exitoso. Ahora puedes iniciar sesión.");
